...
fetchWithCred.interceptors.request.use(
  async (config) => {
    ...
    if (netState.isConnected === false) {
      // Hold the request until connectivity returns. The adapter's result is the replayed
      // request's response; the replay itself passes back through these interceptors.
      const replay = { ...config };
      config.adapter = () => enqueueRequest(replay);
      return config;
    }
...
// Called only from logout(), which owns the `isLoggingOut` guard.
const logoutAction = async () => {
  try {
    return await fetchWithCred.get('/auth/logout', { skipAuth: true });
  } catch (error) {
    if (error?.response) {
