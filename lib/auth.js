...
async function signup(payload) {
  ...
  if (res.ok) {
    ...
    await AsyncStorage.setItem('isRegUser', 'true');
    return { d: data };
  } else {
...
async function login(email, password, role) {
  ...
  if (res.ok) {
    ...
    const user = {...data.user, role} || {...data?.data?.user, role};
...
export { fetchWithCred, signup, login, getAccessToken, getUserData, refreshToken, logout };