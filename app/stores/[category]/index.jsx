...
import { fetchFood,fetchAllStores,fetchGroceries, fetchRestaurants, fetchGroceriesStores, fetchFoodExtras } from "@/services/api";
import { StoreCard, ItemCard} from '@/components/ItemCard';
...
const Category = () => {
  ...
  const fnChoice = useCallback(() => {
    return params.category === 'restaurants' ? fetchRestaurants : params.category === 'grocery-stores'?fetchGroceriesStores:
    params.category === 'groceries'?fetchGroceries:params.category === 'food'?fetchFood:params.category === 'extras'?fetchFoodExtras:fetchAllStores;
  }, [params.category]);
  
  const fetchFn = useCallback(() => fnChoice()({ searchQuery }), [fnChoice, searchQuery]);
  const text = params.category==='all'? 'All Stores':params.category.replace(/-/g, " ")
  const {
    data:items= [],
    loading,
    error,
  ...
  return (
    <View style={styles.container}>
      <Appbar.Header style={{backgroundColor:'#F8F8F8', paddingEnd:16}}>
        ...
        <Appbar.Content title={text} variant="titleMedium" titleStyle={{fontWeight:'700', textTransform:'capitalize'}} />
      ...
      {(params.category ==='restaurants'||params.category==='grocery-stores')?
      <FlatList
        data={items}
        style={{flex:1,paddingHorizontal:12, gap:12}}
        renderItem = {({item}) => <StoreCard data={item} storeType={params.category}/>}
      ...
      />:<FlatList
        ...
        renderItem = {({item}) => <ItemCard data={item} storeType={params.category} />}
        columnWrapperStyle={{
